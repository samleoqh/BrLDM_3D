...
def concat_covariates(_dict):
    ...
    """
    conditions = [
        _dict['followup_age']/100., 
        _dict['followup_time']/1500., 
        _dict['followup_treatment'], 
...
def images_to_tensorboard(
    ...
    for tag_i, i in enumerate(random_indices):
        ...
        starting_z = dataset[i]['starting_latent_path'] * scale_factor
        
        context    = dataset[i]['context']
        starting_a = dataset[i]['starting_time']
...
if __name__ == '__main__':
    ...
    transforms_fn = transforms.Compose([
        transforms.LoadImageD(keys=['starting_latent_path', 'followup_latent_path'], reader=npz_reader), 
        transforms.EnsureChannelFirstD(keys=['starting_latent_path', 'followup_latent_path'], channel_dim=0), 
        transforms.DivisiblePadD(keys=['starting_latent_path', 'followup_latent_path'], k=4, mode='constant'), 
    ...
    trainset_repeated = ConcatDataset([trainset] * 5)
    
    validset_repeated = ConcatDataset([validset] * 1)
    ...
    # optimizer = torch.optim.SGD(controlnet.parameters(), lr=args.lr, momentum=0.9)
    ...
    for epoch in range(args.n_epochs):
        ...
        for mode in loaders.keys():
            ...
            for step, batch in progress_bar:
                ...
                with torch.set_grad_enabled(mode == 'train'):
                    ...
                    starting_a = batch['starting_time'].to(DEVICE)
                    ...
                    with torch.autocast('cuda', enabled=True): 
                        ...
                        # if  mode != 'train':
                        # print(followup_z.shape, timesteps.shape, context.shape, controlnet_condition.shape)
                        if  mode != 'train': 
            ...
            if best_tr_loss > epoch_loss:
                best_tr_loss = epoch_loss
                savepath = os.path.join(args.output_dir, f'cnet-ep-{epoch}-tr_{best_tr_loss:.3f}.pth')
        ...
        if best_loss > epoch_loss:
            ...
            savepath = os.path.join(args.output_dir, f'cnet-ep-{epoch}-val_{best_loss:.3f}.pth')
            torch.save(controlnet.state_dict(), savepath)